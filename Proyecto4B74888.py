...
def modulador_16QAM(bits, fc, mpp):
    '''Un método que simula el esquema de 
    modulación digital 16QAM.
'''
    #  Parámetros de la 'señal' de información (bits)
    N = len(bits) # Cantidad de bits
    S = int(len(bits)/4) # Cantidad de simbolos
    
    # Periodo de la señal portadora.
    Tc = 1 / fc  # periodo [s]
    t_periodo = np.linspace(0, Tc, mpp)  # mpp: muestras por período
    
    portadora_seno = np.sin(2*np.pi*fc*t_periodo)
    portadora_coseno = np.sin(2*np.pi*fc*t_periodo)
    portadora = [portadora_seno,portadora_coseno]
    
    # Señal modulada.
    t_simulacion = np.linspace(0, S*Tc, S*mpp) 
    senal_Tx_quad = np.zeros(t_simulacion.shape)
    senal_Tx_fase = np.zeros(t_simulacion.shape)
    moduladora = np.zeros(t_simulacion.shape) 
 
    # Formas de onda según los simbolos.
    for i in range(0,S): #Recorro los simbolos disponibles
        b1=False #El bit 1 del simbolo es 0
        b2=False #El bit 2 del simbolo es 0
        b3=False #El bit 3 del simbolo es 0
        b4=False #El bit 4 del simbolo es 0
        aporte_modulado=0
        #moduladora[i*mpp : (i+1)*mpp] = 1
        #Ubicación de los bits del stream de datos
        q_f=i*4
        for q in  range(q_f,q_f+4):
            ...
            if q==q_f:
                ...
                if bits[q]==1:
                    b4=True
        #Asigno para la portadora en fase
        if b1==False and b2==False:
            modulado_coseno=portadora_coseno * -3
            aporte_modulado+=0
        elif b1==False and b2==True:
            modulado_coseno=portadora_coseno * -1
            aporte_modulado+=4
            ...
            modulado_coseno=portadora_coseno * 3
            aporte_modulado+=8
        #Asigno para la portadora en quadratura
        if b3==False and b4==False:
            modulado_seno=portadora_coseno * 3
            aporte_modulado+=0
        elif b3==False and b4==True:
            modulado_seno=portadora_coseno * 1
            aporte_modulado+=1
        elif b3==True and b4==True:
            modulado_seno=portadora_coseno * -1
            aporte_modulado+=3
        elif b3==True and b4==False:
            modulado_seno=portadora_coseno * -3
            aporte_modulado+=2
        moduladora[i*mpp : (i+1)*mpp] = aporte_modulado   
        senal_Tx_fase[i*mpp : (i+1)*mpp]=modulado_coseno
        senal_Tx_quad[i*mpp : (i+1)*mpp]=modulado_seno
    

    senal_Tx=np.stack([senal_Tx_fase, senal_Tx_quad], axis=0)
    # 5. Calcular la potencia promedio de la señal modulada
    P_senal_Tx_quad = (1 / (S*Tc)) * np.trapz(pow(senal_Tx_quad, 2), t_simulacion)
    P_senal_Tx_fase = (1 / (S*Tc)) * np.trapz(pow(senal_Tx_fase, 2), t_simulacion)
    P_senal_Tx = P_senal_Tx_fase+P_senal_Tx_quad
    
    
    return senal_Tx, P_senal_Tx, portadora,moduladora

def demodulador_16QAM(senal_Rx, portadora, mpp):
    '''Un método que simula un bloque demodulador de señales, bajo un esquema 16QAM. 
    '''
    #Descompongo la senal en fase y quad
    senal_Rx_fase=senal_Rx[0,:]
    senal_Rx_quad=senal_Rx[1,:]

    
    # Muestras en senal_Rx_fase
    
    M = len(senal_Rx_fase)
    # Cantidad de bits en transmisión
    S = int(M / mpp)
    # Vector para bits obtenidos por la demodulación
    bits_Rx = np.zeros(S*4)
    #print(bits_Rx.shape)
    # Vector para la señal demodulada
    senal_demodulada = np.zeros(senal_Rx_fase.shape)
    Es = np.sum(portadora[1] * portadora[1])
    
    # Demodulación
    for i in range(0,S):
        # Producto interno de dos funciones
        producto_fase = senal_Rx_fase[i*mpp : (i+1)*mpp] * portadora[0]
        A_fase = np.sum(producto_fase)/Es 
        producto_quad = senal_Rx_quad[i*mpp : (i+1)*mpp] * portadora[1]
        A_quad = np.sum(producto_quad)/Es 
        # Detección de energía
        aporte_modulado=0
        #Primeros 2 bits del simbolo de la senal en fase
        q=4*i
        if A_fase >= 2:
            bits_Rx[q] = 1 #b1
            bits_Rx[q+1] = 0 #b2
        ...
        #Últimos 2 bits del simbolo de la senal en fase
        if A_quad >= 2:
            bits_Rx[q+2] = 0 #b3
            bits_Rx[q+3] = 0 #b4
            aporte_modulado+=0

        elif A_quad >= 0 and A_quad < 2:
            bits_Rx[q+2] = 0 #b3
            bits_Rx[q+3] = 1 #b4
            aporte_modulado+=1
            ...
            bits_Rx[q+3] = 0 #b4
            aporte_modulado+=2
        senal_demodulada[i*mpp : (i+1)*mpp] = aporte_modulado
...
# Importar y convertir la imagen a trasmitir
imagen_Tx = fuente_info('arenal.jpg')
dimensiones = imagen_Tx.shape

# Codificar los pixeles de la imagen transmitida
bits_Tx = rgb_a_bit(imagen_Tx)
bits_Tx_muestra=bits_Tx[0:8]

# Modular cadena de bits usando 16QAM
senal_Tx, Pm, portadora,moduladora_tx = modulador_16QAM(bits_Tx, fc, mpp)

# Transmisión de la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, Pm/2, SNR)

# Se desmodula la señal recibida del canal con 16QAM
...
# La onda cuadrada moduladora (bits de entrada)
ax1.plot(moduladora_tx[0:600], color='r', lw=2) 
ax1.set_ylabel('$b(t)$')

# La señal modulada por 16QAM
ax2.plot(senal_Tx[0,0:600], color='g', lw=2) 
ax2.plot(senal_Tx[1,0:600], color='c', lw=2) 
ax2.set_ylabel('$s(t)$')

# La señal modulada al dejar el canal
ax3.plot(senal_Rx[0,0:600], color='b', lw=2)
ax3.plot(senal_Rx[1,0:600], color='m', lw=2)
ax3.set_ylabel('$s(t) + n(t)$')
...
senal_f_fase = fft(senal_Tx[0,:])
senal_f_quad = fft(senal_Tx[1,:])
# Muestras de la señal
Nm = len(senal_Tx[0,:])

# Número de símbolos (198 x 89 x 8 x 3)/4
Ns = (Nm // mpp)/4
...
potencia=2.0/Nm * np.power(np.abs(senal_f_fase[0:Nm//2]),2)+2.0/Nm * np.power(np.abs(senal_f_quad[0:Nm//2]),2)
# Gráfica
plt.plot(f, potencia)
